...
/// Concatenates tracks by overlapping the audios
...
mod test {
    ...
    fn join_tracks_test() {
        let tracks = vec![
            vec![1., 1., 0., 0.5, 0.3].as_frames(),
            vec![0., 1., 0., 0.5, 0.5].as_frames(),
        ];

        assert_eq!(vec![1.0, 2., 0., 1.0, 0.8].as_frames(), join_tracks(tracks))
    }
}
